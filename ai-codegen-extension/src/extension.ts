...
function bustForIframe(u: string): string {
  try {
    const url = new URL(u);
...
async function waitForBackendHealth(attempts = 8, intervalMs = 300): Promise<boolean> {
  try {
    const base = vscode.workspace.getConfiguration(SETTINGS_NS).get<string>("backendBaseUrl");
    if (!base) return true;
...
function ensurePanel(context: vscode.ExtensionContext): vscode.WebviewPanel {
  if (!currentPanel) {
    ...
    currentPanel.onDidDispose(async () => {
      ...
      const zenApplied = (extCtxRef ?? context).globalState.get<boolean>(STORAGE_KEYS_UI.zenApplied, false);
      if (zenApplied) {
        try { await vscode.commands.executeCommand("workbench.action.toggleZenMode"); } catch {}
        try { await (extCtxRef ?? context).globalState.update(STORAGE_KEYS_UI.zenApplied, false); } catch {}
      }
    });
    ...
    currentPanel.webview.onDidReceiveMessage(async (msg: any) => {
      ...
      if (msg?.type === "placementAccepted" && msg?.payload) {
        try {
          ...
          const p = msg.payload;
          const issues: string[] = [];
          if (p?.ar?.deltaPct > 0.02) issues.push(`AR-avvikelse ${(p.ar.deltaPct * 100).toFixed(1)}%`);
          if (p?.sizePct < 0.15) issues.push("Overlay < 15% av ytan");
          const near = p?.edges ? Object.entries(p.edges).filter(([, v]) => v).map(([k]) => k).join(", ") : "";
          const hint = near ? `Nära kanter: ${near}` : "";

          if (issues.length) vscode.window.showWarningMessage(`Placement kontrollerad: ${issues.join(" · ")} ${hint}`);
      ...
      if (msg?.cmd === "acceptCandidate") {
        ...
        await rememberCandidate(pendingCandidate, extCtxRef ?? ({} as any));
...
/* ─────────────────────────────────────────────────────────
   ...
   ───────────────────────────────────────────────────────── */
function resolvePreferredDir(): string | null {
  const cfg = vscode.workspace.getConfiguration(SETTINGS_NS);
  const raw = (cfg.get<string>("preferredPreviewPath", "") || "").trim();
  if (raw) {
    const abs = path.isAbsolute(raw)
      ? raw
      : (vscode.workspace.workspaceFolders || [])
          .map(f => path.join(f.uri.fsPath, raw))
          .find(p => { try { return fs.existsSync(p) && fs.statSync(p).isDirectory(); } catch { return false; } }) || "";
    if (abs && fs.existsSync(abs) && fs.statSync(abs).isDirectory()) return abs;
  }
  // Fallback: mappnamn
  const name = (cfg.get<string>("preferredPreviewDirName", "frontendplay") || "frontendplay").toLowerCase();
...
function preferCandidate(cands: Candidate[]): Candidate | null {
  ...
  if (preferredDir) {
    ...
    const byName = cands.find(c =>
      path.basename(c.dir).toLowerCase() === base || (c.pkgName || "").toLowerCase() === base
    );
    if (byName) return byName;
    // Om detectProjects inte hittade den, returnera minimal kandidat för smart-autostart
    return { dir: preferredDir } as Candidate;
  }
  // Ingen preferens: högst confidence → första
  return cands.sort((a,b)=> (b.confidence ?? 0) - (a.confidence ?? 0))[0] ?? null;
}
...
async function startOrRespectfulFallback(
...
): Promise<{ localUrl: string; externalUrl: string; mode: "dev" | "http" | "inline"; watchRoot?: string }> {
  try {
    log("Startar smart server", { cwd: redactPath(c.dir) });
    const { localUrl, externalUrl } = await runSmartServer(c.dir, selectLaunchCommand(c));
    try {
  ...
  if (cmd) {
    try {
      ...
      const { localUrl, externalUrl } = await runDevServer(cmd, c.dir);
  ...
  if (html) {
    ...
    const { localUrl, externalUrl } = await runInlineStaticServer(html.root);
    const baseExt = externalUrl.endsWith("/") ? externalUrl : externalUrl + "/";
    const baseLoc = localUrl.endsWith("/") ? localUrl : localUrl + "/";
    const eurl = baseExt + encodeURI(html.relHtml);
    const lurl = baseLoc + encodeURI(html.relHtml);
  ...
  const { localUrl, externalUrl } = await runInlineStaticServer(storageDir);
      ...
      const req = mod.request(
        ...
        (res) => {
          ...
          res.on("end", () => {
            ...
            if ((res.statusCode ?? 500) >= 400) return reject(new Error(`${res.statusCode} ${res.statusMessage}: ${body}`));
  ...
  const openCmd = vscode.commands.registerCommand("ai-figma-codegen.openPanel", async () => {
    ...
    if (!pendingCandidate) {
      ...
      if (candidates.length) {
        ...
        const picked = preferCandidate(candidates)!;
        pendingCandidate = picked;
        if (preferredDir || candidates.length === 1 || (pendingCandidate?.confidence ?? 0) >= AUTO_START_SURE_THRESHOLD) {
          lastUiPhase = "loading";
          panel.webview.postMessage({ type: "ui-phase", phase: "loading" });
          await rememberCandidate(pendingCandidate!, context);
          await tryAutoFullView(panel, context);
