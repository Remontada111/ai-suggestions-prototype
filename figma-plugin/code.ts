...
async function main() {
  ...
  if (figma.currentPage.selection.length === 0) {
    figma.notify("Välj minst en nod"); figma.closePlugin(); return;
  }
  ...
  if (!figma.fileKey) {
    figma.notify("Filen är inte synkad till molnet"); figma.closePlugin(); return;
  }
  ...
  try {
    ...
    figma.notify("API-fel: " + (err as Error).message);
