...
- Textnoder: om 'text.lines' finns → en rad per <div>, annars 'text.content'.
...
def _text_classes(st: Dict[str, Any]) -> List[str]:
    ...
    if isinstance(ls, str) and ls.strip():
        ...
        if m:
            ...
            # okänt format/procent → neutrum (valideringen tillåter tracking-normal)
    ...
    if isinstance(fam, str) and fam.strip():
        fam_us = fam.strip().replace(" ", "_").replace("\\", "\\\\").replace("'", "\\'")
        out.append(f"font-['{fam_us}']")
...
def _escape_for_jsx_text(s: str) -> str:
    ...
    JSX-special (<, >, &) behöver inte extra escaping i en template-literal.
    """
...
def _emit_text_lines(n: Dict[str, Any]) -> List[str]:
    """Returnerar en lista av rader för TEXT-noder (en <div> per rad)."""
    t = n.get("text") or {}
    ...
    if isinstance(lines, list) and len(lines) > 0:
        for s in lines:
            s = _escape_for_jsx_text(str(s or ""))
            if s:
                out.append(s)
    elif content:
        out.append(_escape_for_jsx_text(content))
    return out
...
def _classes_for_node(n: Dict[str, Any]) -> str:
    """
    Kombinera:
      - tw.list (figma_ir.py),
      - text.style (för TEXT),
    ...
    return _sanitize_tw(" ".join(_dedup(base)))
...
def _build_icon_map(icon_assets: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    ...
    for ia in icon_assets or []:
        ...
        if not path.lower().endswith(".svg") and not path.lower().endswith(".svg?url"):
            # Endast SVG-stöd i denna path; andra format kan implementeras senare
            continue
...
def _emit_node(
    ...
    if nid and nid in icon_map:
        ...
        w = max(w, 1)  # säkerställ >0 (validering kommer annars klaga)
        h = max(h, 1)
    ...
    # TEXT: rendera rader separat om de finns
    if n.get("type") == "TEXT":
        cls = _classes_for_node(n)
        rows = _emit_text_lines(n)
        if rows:
            lines.append(f"{indent}<div className='{cls}'>")
            for s in rows:
                lines.append(f"{indent}  <div>{{`{s}`}}</div>")
    ...
    for ch in (n.get("children") or []):
        _emit_node(ch, lines, icon_map, import_vars, used_names, depth + 1)
    lines.append(f"{indent}</div>")
...
def generate_tsx_component(
    ...
    _emit_node(ir["root"], body_lines, icon_map, import_vars, used_names, depth=2)
