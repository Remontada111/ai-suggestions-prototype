...
function resolveModuleFile(fromFile: string, importPath: string): string | null {
  ...
  for (const ext of [".tsx", ".ts", ".jsx", ".js"]) add(importPath + ext);
  for (const idxExt of [".tsx", ".ts", ".jsx", ".js"]) add(path.join(importPath, "index" + idxExt));
...
function hasImportForLocal(code: string, localName: string, importPath: string): boolean {
  ...
  const reFrom = new RegExp(
    String.raw`(^|\n)\s*import\s+([^;]*?)\s+from\s+["\']${escapeRegex(spec)}["\']`,
    "g",
  );
...
function listImportedIdents(code: string): Set<string> {
  ...
  while ((m = re.exec(code))) {
    ...
    for (const t of named
      ...
      .map((s) => s.trim())
      .filter(Boolean)) {
      const alias = /as\s+([A-Za-z_$][\w$]*)$/.exec(t)?.[1] || /^([A-Za-z_$][\w$]*)$/.exec(t)?.[1] || null;
...
function wrapInnerWithGrid(
  code: string,
  anchor: AnchorPaired,
): { code: string; anchor: AnchorPaired } {
...
function logTileInventory(tag: string, code: string) {
  const region =
    /\{\/\*\s*AI-INJECT-MOUNT:BEGIN\s*\*\/\}([\s\S]*?)\{\/\*\s*AI-INJECT-MOUNT:END\s*\*\/\}/.exec(code)?.[1] || "";
  const marked = Array.from(region.matchAll(/\{\s*\/\*\s*(AI-TILE:[^*]+):BEGIN\s*\*\/\}/g)).map((m) => m[1]);
  const legacy = Array.from(
    region.matchAll(/<div className="relative [^>]+>[\s\S]*?<([A-Z]\w*)\b[\s\S]*?<\/div>/g),
  ).map((m) => m[1]);
...
/** NEW: Remove any marked tiles with keys from `keys` that live OUTSIDE the paired region. */
function purgeStrayMarkedTilesOutsideAnchor(code: string, anchor: AnchorPaired, keys: string[]): string {
  ...
  for (const key of keys) {
    ...
    const matches = Array.from(out.matchAll(blockRe));
    if (!matches.length) continue;
    // Remove from end to start to keep indices stable
    for (let i = matches.length - 1; i >= 0; i--) {
      const m = matches[i];
      const absIndex = m.index ?? -1;
      if (absIndex < 0) continue;
      if (absIndex < anchor.beginEnd || absIndex > anchor.endStart) {
        console.log("[ai_inject_mount] PURGE stray marked tile outside anchor", { key });
        out = out.slice(0, absIndex) + out.slice(absIndex + m[0].length);
      }
    }
  }
  return out;
...
function replaceMarkedTile(
  ...
  for (const key of keys) {
    ...
    const updated = code.slice(0, absOpen) + openStr + newTileContent + closeStr + code.slice(absCloseEnd);
...
function parseImportLocals(clause: string): string[] {
  ...
  for (const t of named
    ...
    .filter(Boolean)) {
    const alias =
      /as\s+([A-Za-z_$][\w$]*)$/.exec(t)?.[1] || /^([A-Za-z_$][\w$]*)$/.exec(t)?.[1] || null;
...
function dropTilesWhoseImportMissing(code: string): string {
  ...
  const hasImport = (ident: string) => {
    const re = new RegExp(String.raw`(^|\n)\s*import\s+([^;]*?\b${ident}\b[^;]*?)\s+from\s+['"][^'"]+['"]\s*;`, "m");
    return re.test(code);
  };

  // Marked tiles
  const tileRe =
    /\{\s*\/\*\s*AI-TILE:([^\*]+):BEGIN\s*\*\/\}([\s\S]*?)\{\s*\/\*\s*AI-TILE:\1:END\s*\*\/\}/g;
...
/** NEW: keep at most one tile with a given key inside anchor. Prefer the last occurrence. */
function dedupeMarkedTilesInsideAnchor(code: string, anchor: AnchorPaired, key: string): string {
  const inner = code.slice(anchor.beginEnd, anchor.endStart);
  const re = new RegExp(
    String.raw`\{\s*/\*\s*${escapeRegex(key)}\s*:\s*BEGIN\s*\*/\}[\s\S]*?\{\s*/\*\s*${escapeRegex(
      key,
    )}\s*:\s*END\s*\*/\}`,
    "g",
  );
  const matches = Array.from(inner.matchAll(re));
  if (matches.length <= 1) return code;

  const keepIndex = matches.length - 1; // keep last
  let cursor = 0;
  let newInner = "";
  ...
  for (let i = 0; i < matches.length; i++) {
    ...
    const idx = m.index ?? 0;
    if (i === keepIndex) {
      newInner += inner.slice(cursor, idx) + m[0];
      cursor = idx + m[0].length;
    } else {
      newInner += inner.slice(cursor, idx);
      cursor = idx + m[0].length;
    }
  }
  newInner += inner.slice(cursor);

  console.log("[ai_inject_mount] DEDUPE inside anchor", { key, kept: "last", removed: matches.length - 1 });
  return code.slice(0, anchor.beginEnd) + newInner + code.slice(anchor.endStart);
}
...
function main(): void {
  ...
  // direct log right after argv + originalCode
  {
  ...
  // just before write
  {
    const innerMatch = nextCode.match(
      /\{\/\*\s*AI-INJECT-MOUNT:BEGIN\s*\*\/\}([\s\S]*?)\{\/\*\s*AI-INJECT-MOUNT:END\s*\*\/\}/,
    );
    console.error(
      JSON.stringify({
        tag: "inject.apply",
