...
function findAnchorRange(code: string): { start: number; end: number; indent: string } | null {
  // Supports: {/* AI-INJECT-MOUNT */}, // AI-INJECT-MOUNT, /* AI-INJECT-MOUNT */
  const patterns = [
...
function main(): void {
  ...
  dbg('hasImportBefore', hasImportFor(originalCode, importName, importPath));
  ...
  dbg('anchor', anchor);
  if (!anchor) fail('Could not find AI-INJECT-MOUNT anchor in main file.');
